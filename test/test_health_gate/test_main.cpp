...
void test_only_complete_temporary_health_promotes() {
  State state = healthy();
  TEST_ASSERT_TRUE(mindflayer::health::shouldPromote(state));
  bool* fields[] = {&state.provisioning,
                    &state.normalMode,
                    &state.wifi,
                    &state.pinnedTls,
                    &state.wss,
                    ...
                    &state.acceptedVersionMatches};
  for (auto field : fields) {
    *field = false;
    TEST_ASSERT_FALSE(mindflayer::health::shouldPromote(state));
    *field = true;
  }
...
int main() {
  UNITY_BEGIN();
  RUN_TEST(test_only_complete_temporary_health_promotes);
  RUN_TEST(test_recovery_is_armed_only_for_permanent_boots);
  return UNITY_END();
}

...
def assert_identity(python,esptool,port):
    ...
    output=run(python,esptool,port,'read_mac',capture=True)
...
def main():
    ...
    p.add_argument('--backup-dir',required=True);p.add_argument('--python',default=sys.executable)
    p.add_argument('--esptool',default=os.path.expanduser('~/.platformio/packages/tool-esptoolpy/esptool.py'));a=p.parse_args()
    assert_identity(a.python,a.esptool,a.port); backup=pathlib.Path(a.backup_dir);backup.mkdir(parents=True,exist_ok=False)
    before=[]
    for address,name in PROVISIONING:
        target=backup/name;run(a.python,a.esptool,a.port,'read_flash',hex(address),'0x1000',target);before.append(digest(target))
    assert_identity(a.python,a.esptool,a.port)
    run(a.python,a.esptool,a.port,'write_flash','--flash_mode','dio','--flash_freq','40m','--flash_size','4MB',
        '0x000000',a.rboot,'0x001000',a.metadata_a,'0x100000',a.metadata_b,'0x002000',a.slot_a)
    after=[]
    for address,name in PROVISIONING:
        target=backup/('after-'+name);run(a.python,a.esptool,a.port,'read_flash',hex(address),'0x1000',target);after.append(digest(target))

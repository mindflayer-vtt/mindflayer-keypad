...
Geometry geometry(Slot slot) {
  if (slot == Slot::A) return {kSlotAAddress, kSlotSize};
  if (slot == Slot::B) return {kSlotBAddress, kSlotSize};
  return {0, 0};
}
...
bool validAbsoluteRange(Slot slot, uint32_t address, uint32_t length) {
  const Geometry value = geometry(slot);
  return value.size && address >= value.start &&
         validRange(slot, address - value.start, length);
}

Writer::Writer(Flash& flash, Slot active, Slot permanent)
    : flash_(flash), active_(active), permanent_(permanent), target_(Slot::Invalid), targetGeometry_{0, 0},
      imageSize_(0), received_(0), written_(0), word_{0xff, 0xff, 0xff, 0xff},
      wordSize_(0), state_(State::Idle), error_(Error::None) {}
...
bool Writer::begin(Slot target, uint32_t imageSize) {
  if (state_ == State::Writing) return fail(Error::WrongState);
  targetGeometry_ = geometry(target);
  if (!targetGeometry_.size) return fail(Error::InvalidSlot);
  if (target == active_ || target == permanent_) return fail(Error::ActiveSlot);
  if (!imageSize || !validRange(target, 0, imageSize)) return fail(Error::InvalidSize);
  target_ = target;
...
bool Writer::flushWord(bool final) {
  if (wordSize_ < sizeof(word_) && !final) return true;
  if (!wordSize_) return true;
...
bool Writer::write(const uint8_t* data, size_t size) {
  if (state_ != State::Writing) return fail(Error::WrongState);
  if (!data && size) return fail(Error::WrongState);
  if (size > imageSize_ - received_) return fail(Error::TooMuchData);
  if (!wordSize_ && size >= 4) {
  ...
  while (size--) {
    ...
    ++received_;
    if (!flushWord(false)) return false;
  }
  return true;
}
...
bool Writer::validate() {
  uint8_t header[8];
  if (!flash_.read(targetGeometry_.start, header, sizeof(header))) return false;
  if (header[0] != 0xea || header[1] != 0x04) return false;
  uint32_t offset = 8;
  if (!flash_.read(targetGeometry_.start + offset, header, sizeof(header))) return false;
  const uint32_t iromLength = readLe32(header + 4);
  if (readLe32(header) != 0 || iromLength == 0 || iromLength > imageSize_ - 16) return false;
  ...
  while (remaining) {
    ...
    if (!flash_.read(targetGeometry_.start + offset, buffer, chunk)) return false;
    for (uint32_t i = 0; i < chunk; ++i) checksum ^= buffer[i];
    offset += chunk;
    remaining -= chunk;
  }
  offset = (offset + 15) & ~15U;
  if (offset + 8 > imageSize_ ||
      !flash_.read(targetGeometry_.start + offset, header, sizeof(header))) return false;
  ...
  if (!rangeWithin(entry, 1, kIramStart, kIramEnd)) return false;
  offset += 8;
  for (uint8_t section = 0; section < sections; ++section) {
    ...
    if (!length || length > imageSize_ - offset - 8) return false;
    if (!rangeWithin(address, length, kIramStart, kIramEnd) &&
        !rangeWithin(address, length, kDramStart, kDramEnd)) return false;
    offset += 8;
    remaining = length;
    while (remaining) {
      const uint32_t chunk = remaining < sizeof(buffer) ? remaining : sizeof(buffer);
  ...
  if (offset >= imageSize_ || offset + 1 != imageSize_) return false;
  uint8_t stored = 0;
...
bool Writer::finish() {
  if (state_ != State::Writing) return fail(Error::WrongState);
  if (received_ != imageSize_) return fail(Error::InvalidSize);
  if (!flushWord(true)) return false;
  if (!validate()) return fail(Error::InvalidImage);
  state_ = State::Complete;
  return true;
}

void Writer::abort() {
  if (state_ == State::Writing) state_ = State::Failed;

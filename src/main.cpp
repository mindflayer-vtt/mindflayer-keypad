...
public:
  bool read(uint32_t a, void* d, size_t n) override {
    return ESP.flashRead(a, static_cast<uint8_t*>(d), n);
  }
  bool eraseSector(uint32_t s) override { return ESP.flashEraseSector(s); }
  bool write(uint32_t a, const void* d, size_t n) override {
    return ESP.flashWrite(a, const_cast<uint8_t*>(static_cast<const uint8_t*>(d)), n);
  }
};
...
static void clearRecoveryMarker() {
...
static void base64Url(const uint8_t input[32], char output[44]) {
  ...
  size_t in = 0, out = 0;
  while (in + 3 <= 32) {
    uint32_t n = ((uint32_t)input[in] << 16) | ((uint32_t)input[in + 1] << 8) | input[in + 2];
    in += 3;
    output[out++] = alphabet[n >> 18];
    output[out++] = alphabet[(n >> 12) & 63];
    output[out++] = alphabet[(n >> 6) & 63];
    output[out++] = alphabet[n & 63];
  }
  uint32_t n = (uint32_t)input[in] << 16 | (uint32_t)input[in + 1] << 8;
  output[out++] = alphabet[n >> 18];
  output[out++] = alphabet[(n >> 12) & 63];
  output[out++] = alphabet[(n >> 6) & 63];
  output[out] = 0;
...
static void performUpdate(const protocol::UpdateAvailable& update) {
  ...
  HTTPClient http;
  String url = String("https://") + settings.serverHost + ':' + settings.serverPort + update.path;
  ...
  if (temporaryBoot || update.size <= trailerSize) {
  ...
  while (ok && received < update.size) {
    ...
    int got = stream.readBytes(otaBuffer, count);
    if (got <= 0) {
      ok = false;
      break;
    }
    ...
    if (received < imageSize) {
      size_t body = min(count, static_cast<size_t>(imageSize - received));
      firmwareHash.add(otaBuffer, body);
      ok = writer.write(otaBuffer, body);
      offset = body;
    }
    if (ok && offset < count) {
      size_t tailOffset = received + offset - imageSize;
...
static void sendFrame(size_t size) {
...
static void flushFrame() {
  if (pendingFrameSize && client.available()) {
    size_t size = pendingFrameSize;
    pendingFrameSize = 0;
    client.sendBinary((const char*)pendingFrame, size);
  }
}
static void processMessage(const uint8_t* data, size_t size) {
  ...
  if (protocol::parseAuthChallenge(data, size, challenge)) {
    if (protocol::buildAuthResponse(frameBuffer, sizeof(frameBuffer), written, settings.deviceId,
                                    settings.deviceSecret, challenge.challenge))
      sendFrame(written);
    return;
  }
  protocol::AuthResult auth;
  if (protocol::parseAuthResult(data, size, auth)) {
    if (auth.status != protocol::AUTH_OK || strcmp(auth.deviceId, settings.deviceId)) {
    ...
    Serial.printf("Authenticated as %s; firmware=%s; ", settings.deviceId, FIRMWARE_VERSION);
    printHeapStats();
    return;
  }
  ...
  if (authenticated && protocol::parseUpdateAvailable(data, size, update)) {
    ...
    authenticated = wssHealthy = registered = false;
...
#ifdef RBOOT_INTEGRATION
    const mindflayer::health::State health = {temporaryBoot,
                                              provisioned,
                                              true,
                                              wifiHealthy,
  ...
  if (authenticated && protocol::parseConfiguration(data, size, configuration)) {
    setColors(configuration.led1.r, configuration.led1.g, configuration.led1.b,
  ...
  client.close(CloseReason_ProtocolError);
}
...
static void setupWebSocket() {
  ...
  if (!client.connectSecure(settings.serverHost, settings.serverPort, "/device/v1")) {
...
void setup() {
  ...
  temporaryBoot = BootControl::isTemporaryBoot();
  temporaryStarted = millis();
  Serial.printf("rBoot slot=%c permanent=%c mode=%s\n", BootControl::currentSlot() ? 'B' : 'A',
                BootControl::permanentSlot() ? 'B' : 'A',
                temporaryBoot ? "TEMPORARY" : "PERMANENT");
#endif
  provisioning::Selection selected;
  if (!provisioning::loadStored(settings, &selected)) {
    clearRecoveryMarker();
    Serial.println(
  ...
  printHeapStats();
  serverPublicKey =
      new BearSSL::PublicKey(settings.serverPublicKey, settings.serverPublicKeyLength);
  if (!serverPublicKey->isRSA() && !serverPublicKey->isEC()) {
    Serial.println("UNPROVISIONED; invalid server public key");
    return;
  }
  if (mindflayer::health::shouldArmSerialRecovery(temporaryBoot, !temporaryBoot) &&
...
void onKeyChange(KeyboardMatrix::KeyState* key) {
  ...
  if (protocol::buildKeyEvent(frameBuffer, sizeof(frameBuffer), written, key->key, key->isDown))
    client.sendBinary((const char*)frameBuffer, written);
}
void loop() {
  if (!provisioned) {
  ...
  client.poll();
  processWebSocketCallbacks();
  flushFrame();
  if (authenticated && client.available())
    KeyboardMatrix::detectKeys(onKeyChange);
#ifdef RBOOT_INTEGRATION
  if (temporaryBoot && millis() - temporaryStarted > TEMPORARY_HEALTH_TIMEOUT_MS) {
  ...
  if (client.available() && millis() - lastPong > 15000) {
    client.ping();
    lastPong = millis();
  }
}

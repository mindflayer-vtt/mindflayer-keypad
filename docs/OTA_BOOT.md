...
Production uses rBoot 1.4.2 commit `614f33685d0dd990fc4202f2409b0d2365eeaef3` and esptool2 commit `91759a7c4faec1b7bc8410166d22ed92eb90556c`. Builds enable `BOOT_RTC_ENABLED`, `BOOT_BIG_FLASH`, `BOOT_CONFIG_CHKSUM`, and `BOOT_IROM_CHKSUM`; GPIO boot selection is disabled. GCC 10.3.0 and Arduino Core 3.1.2 are the validated toolchain, resolved through pinned PlatformIO platform 4.2.1. `scripts/build-rboot.sh` installs the environment's packages before using the Xtensa compiler, so it works before the first application build. It builds patched snapshots of the two pinned source commits without resetting cached checkouts and rejects a bootloader above 4 KiB. The final proven loader is 2,688 bytes.
...
An unsigned/tampered artifact is never selected. A corrupt/incomplete inactive image is never selected. A structurally valid but unhealthy candidate runs temporarily and returns to the committed slot on software/watchdog reset or after the 90-second health timeout. An SDK one-shot timer is armed immediately after temporary-boot detection, before provisioning loads and Wi-Fi/TLS startup. It runs in SDK context during yielding startup/network waits and calls `system_restart()` directly, without serial output or the continuation-only `ESP.restart()` wrapper. The loop also checks the deadline before any early return. Permanent boots do not arm this timer. The timeout can only reboot; it cannot promote. If the application stops yielding entirely, the hardware/software watchdog remains responsible for reset.
